...
export const Profile: React.FC = () => {
    ...
    const handleCancelBooking = (bookingId: string, hourToRefund: number) => {
        ...
        const updateBookings = bookings.map(b => b.id === bookingId ? {...b, status: 'Cancelled' as const} : b);
    ...
    return(
        <div className="container">
            <div>
                ...
                <div>
                    Использовано лимита: {user.usedHours} из {user.monthlyHoursLimit}
                </div>
            </div>
            <div>
                            ...
                            <tbody>
                                {bookings.map((b) => (
                                    <tr key={b.id}>
                                        ...
                                        <td>
                                            {b.status === 'Confirmed' && (
                                                <button onClick={() => handleCancelBooking(b.id, b.hour)}>Отменить</button>
                                            )}
                                        </td>
                                    </tr>
                                ))}
