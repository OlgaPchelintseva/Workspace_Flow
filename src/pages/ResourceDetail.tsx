...
export const ResourceDetail: React.FC = () => {
    ...
    const totalHourseRequested = state.selectedSlots.reduce((sum, slot) => sum + slot.priceHours, 0);

    const handleConfirmBooking = () => {
        if (totalHourseRequested === 0) return;
        const success = deductHours(totalHourseRequested);
        if (success){
            ...
            <div>
                        ...
                        <div>
                            {state.selectedSlots.map(slot => (
                                <div key={slot.id}>
                                    ...
                                    <button onClick={() => dispatch({type: 'TOGGLE_SLOT', payload: slot})}>Удалить</button>
                                </div>
                            ))}
                        </div>
                    )}
                ...
                <div>
                    <div>
                        ...
                        <span>{totalHourseRequested}</span>
                    </div>
                    <button
