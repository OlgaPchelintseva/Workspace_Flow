...
export const BookingFilters: React.FC<BookingFiltersProps> = ({filters, setFilters}) => {
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-gray-500 uppercase">Дата бронирования</label>
                <input 
                    type="date"
                    value={filters.date}
            ...
            <div className="flex flex-col gap-1">
                <label className="text-xs font-semibold text-gray-500 uppercase">Тип ресурса</label>
            ...
            <div ref={dropdownRef} className="relative flex flex-col gap-1">
                <label className="text-xs font-semibold text-gray-500 uppercase">Этаж</label>
                <button
                    onClick={() => setIsFloorOpen(!isFloorOpen)}
                    className="w-full text-left px-4 py-2 border border-gray-200 rounded-lg bg-white focus^outline-none focus:ring-2 focus:ring-indigo-500
